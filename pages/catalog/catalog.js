  ...
  getCurrentCategory() {
    ...
    util.request(api.CatalogCurrent, {
      ...
      if (res.code === 0) {
        ...
        if (Object.keys(res.data.gdMap).length === 0) {
          this.setData({
          ...
          const subjectArray = [];
          Object.keys(res.data.scMap).map((d, i) => {
            if (res.data.gdMap[d]) {
              const temp = {
                goodsItem: res.data.gdMap[d],
                className: Object.values(res.data.scMap)[i],
                classId: Object.keys(res.data.scMap)[i],
              }
              subjectArray.push(temp);
            }
          })
          this.setData({
            goodsList: subjectArray,
            currentSubCategoryList: Object.keys(res.data.scMap).map((e, i) => { return { key: e, name: Object.values(res.data.scMap)[i]} }),
            hasmoreData: false,
          }, () => {
            console.log(51251515, Object.keys(res.data.scMap).map((e, i) => { return { key: e, name: Object.values(res.data.scMap)[i]} }))
          })
        }
      }
      wx.hideLoading();
